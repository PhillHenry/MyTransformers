"""What StockCnn.py and StockLstm.py share: the data, the labels, training and evaluation.

Each of those scripts supplies a model and calls `main`; everything else lives here.

Reads a CSV of `timestamp,open,high,low,close,volume`, turns it into overlapping
windows of scale-free features, and trains a classifier on the binary
...
def main(make_model, description: str):
    """Parse the command line, then build, train and evaluate `make_model(in_channels)` on it.

    `description` is the calling script's own docstring; the notes on the data here
    follow it in --help.
    """
    ...
    parser.add_argument("--window", type=int, default=64,
                        help="minutes of history the model sees; bars are merged into 1-minute bars as "
    ...
    model = make_model(splits[0][0].shape[1]).to(device)
